...
import { ArrowUp, ArrowDown, X, MonitorPlay } from "lucide-react";
    ...
    <div className="bg-white rounded-2xl border mb-6">
      <div className="p-6 border-b">
        ...
        <p className="text-sm text-gray-500 mt-1">
          ...
          <Link href="/admin/products" className="text-primary-600 hover:underline">Products</Link>. Each one shows its name, description, features and photos automatically.
        </p>
      </div>
      {products.length === 0 ? (
          ...
          {products.map((p, i) => (
            <li key={p.id} className="flex items-center gap-4 px-6 py-3">
              ...
              <div className="relative w-12 h-12 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                {p.image && <Image src={p.image} alt="" fill sizes="48px" className="object-contain p-1" />}
              </div>
              <Link href={`/admin/products/${p.id}`} className="flex-1 min-w-0 text-sm font-medium text-gray-900 truncate hover:text-primary-600">{p.name}</Link>
              <div className="flex items-center gap-1">
                <button type="button" disabled={saving || i === 0} onClick={() => move(i, -1)} aria-label="Move up" className="p-2 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg disabled:opacity-30">
                  <ArrowUp size={15} />
                </button>
                <button type="button" disabled={saving || i === products.length - 1} onClick={() => move(i, 1)} aria-label="Move down" className="p-2 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg disabled:opacity-30">
                  <ArrowDown size={15} />
                </button>
